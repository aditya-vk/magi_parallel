...
class ParallelPlanner(Planner):
    ...
    # Same as DFS but has an extra function that spawns parallel processes and performs DFS in each.
    ...
    def plan_action(self, env, action, output_queue=None):
        ...
        # Spawn 'roots' number of parallel processes and run the following code on each of them parallelly
#######################
        ...
        for node_id in roots
            p = Process(target = _plan_action_parallel, args = (env, node_id, num_attempts))
            processes.append(p)
            p.start()

        # Collect the output queues
        resultQ = {}
        for i in roots
            resultQ.update(output_queue.get())

        # For now let all processes finish 
        for p in processes
            p.join()

        return resultQ      # This is going to give all the three solutions. We want only one. Need to terminate.
        
########################

    def _plan_action_parallel(self, env, node_ids, num_attempts):
        ...
        if self.keep_trying:
            ...
            solutions = self._plan_recursive(env, roots, num_attempts)
...
########################
    ...
    def _plan_recursive(self, env, node_ids, num_attempts):
        ...
        import numpy as np
        node_order = np.random.permutation(len(node_ids))   # There is only one node in the root currently
